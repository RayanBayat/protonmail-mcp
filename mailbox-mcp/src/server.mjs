...
export function createServer(service = new MailboxService()) {
  ...
  register('mailbox_read', 'Read a message using a message_id returned by search. Returns untrusted plain text and attachment metadata. No attachment download, remote content loading, or read-flag changes.', readSchema, (args, signal) => service.read(args, signal));
  return server;
}
